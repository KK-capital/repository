...
```
  ...
  └─────────┼────────────────────────────┼─────────────────┘
            ...
            └──────────────┬─────────────┘
                ┌──────────┴───────────┐
                │   F5 Load Balancer   │  TLS 1.3 終結
                │   Sticky Session     │  Health Check /health/live
                └──────────┬───────────┘
                           │ 流量分配
                  ┌────────┴────────┐
                  ▼                 ▼
  ...
  │  │ ASP.NET Core │  │ ASP.NET Core │  32C / 64G           │
  │  │ OmsEngine    │  │ OmsEngine    │  系統碟 100G NVMe    │
  │  │ SignalR Hub  │  │ SignalR Hub  │  資料碟 1TB NVMe     │
  │  │ 10 Gbps      │  │ 10 Gbps      │                     │
  │  └──────┬───────┘  └───────┬──────┘                     │
  │         └─────────┬────────┘                            │
  │                   │                                      │
  │  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐  │
  │  │ FIX Gateway  │  │ Market Data  │  │ Algo Server  │  │
  │  │ #1 Active    │  │ #1 Active    │  │ #1 Active    │  │
  │  │ #2 Standby   │  │ #2 Standby   │  │ #2 Standby   │  │
  │  │ 16C / 32G    │  │ 16C / 32G    │  │ 16C / 32G    │  │
  │  │ 系統碟 100G  │  │ 系統碟 100G  │  │ 系統碟 100G  │  │
  │  │ 資料碟 500G  │  │ 資料碟 500G  │  │ 資料碟 500G  │  │
  ...
  └───────────────────────────┼─────────────────────────────┘
                   ...
                   ▼                      ▼
  ┌─────────────────────┐   ┌──────────────────────────┐
  │ RabbitMQ Cluster ×3 │   │    SQL Server 2022        │
  │ Quorum Queue (Raft) │   │    1 Primary              │
  │ 8C / 32G            │   │    2 Secondary            │
  │ 資料碟 200G NVMe    │   │    Always On AG           │
  │ 10 Gbps             │   │    32C / 128G             │
  └─────────────────────┘   │    資料碟 2TB NVMe        │
                            │    10 Gbps                │
                            └──────────────────────────┘
  ...
  ┌──────────────────────────────────────────────────────────┐
  │  監控主機                                8C / 16G        │
  │  Prometheus + Grafana + OTLP Collector  資料碟 500G NVMe │
  │  抓取所有服務 Metrics / Trace / Log      1 Gbps          │
  └──────────────────────────────────────────────────────────┘
```
