...
| **資料碟** | 200 GB SSD | 500 GB NVMe SSD | 1 TB NVMe SSD | 同正式環境 |
...
| **Log 碟** | — | 200 GB NVMe SSD | 500 GB NVMe SSD | 同正式環境 |
...
| **資料安全** | Quorum Queue 訊息寫入即複製至 3 節點，過半節點存活即保證不遺失；多數節點同時故障時理論窗口 ~20ms |

**吞吐量計算：**

> 每批 2,500 筆 / 20ms 間隔 = **50 批/sec × 2,500 = 125,000 orders/sec** (單節點理論上限)
> AA 兩台合計理論上限 250,000 orders/sec，**實際系統吞吐上限由 DB 寫入能力決定（約 50K/sec），App Server 理論吞吐僅為上界**。
...
| 資料安全 | 即時落碟 | Quorum Queue 3 節點複製，**過半節點存活即保證不遺失；多數節點同時故障時理論窗口 ~20ms** |

### 5.2 RabbitMQ 訊息匯流排架構

RabbitMQ 在本系統中同時承擔兩種角色：**(1) 訂單處理佇列**（高可靠批次寫入）與 **(2) 系統事件匯流排**（元件間解耦通訊）。所有元件之間的通訊皆透過 RabbitMQ 解耦：
...
正式環境與 HA 環境均採用 3 台 RabbitMQ Quorum Queue 部署。
...
| 硬體故障（主機掛掉） | 數分鐘～數小時 | 單節點故障不遺失（Quorum 仍成立）；多數節點同時故障時可能遺失（理論窗口 ~20ms ≈ 最多 2,500 筆） | App Server 降級處理 |
...
> 50,000 orders/sec × 21,600 sec (6hr) = **1,080,000,000 筆/日** (理論峰值，100% 滿載)
> 實際平均負載依場景估算：
> - **高頻 / Algo 場景**（程式單為主）：峰值利用率約 5–20%，預估 **~50M – 200M 筆/日**
> - **法人 / 人工下單場景**（開收盤集中委託）：峰值利用率約 1–5%，預估 **~10M – 50M 筆/日**
...
> ⚠️ 上述為極端 worst-case：單一使用者訂閱全部事件。實務上必須搭配 SignalR Group 訂閱過濾，否則架構在頻寬層面不可行。
...
> **訂單批次寫入說明**：系統不採逐筆即時寫入 DB，而是將訂單先暫存於 RabbitMQ 佇列，由 OmsEngine 定時批次取出並一次性寫入 DB。10K/sec 規格採較大 flush 間隔（50ms / 500 筆），以降低 DB 壓力為優先，延遲容忍度較高；50K/sec 規格縮短 flush 間隔（20ms / 2,500 筆），以吞吐量優先。批次寫入可將 DB 寫入次數從 50,000 次/秒降至 50 次/秒，大幅降低 DB I/O 壓力，同時透過 RabbitMQ Quorum Queue 三節點複製確保訂單不遺失。
...
現行架構正式環境 + HA 共 **16 台**（含 F5），以下依合併程度列出三種方案。各方案均保留 F5 × 1，規格由設備型號決定。

### 9.1 現況盤點

| 主機 | 台數 | 角色 | 合併可能性 |
|------|:----:|------|-----------|
| F5 硬體 LB | 1 | TLS 終結 / Sticky Session / Health Check | 硬體設備，不適用合併 |
...
| 監控主機 | 1 | Prometheus + Grafana + OTLP | 不建議合併，獨立監控更可靠 |
| **合計** | **16** | | |

> **獨立部署為最佳實務**，各服務擁有獨立資源與故障邊界。下列合併方案為成本優化選項，需承擔資源競爭與故障隔離降低的風險，建議依預算與 SLA 需求選擇。
...
> **總結建議**：優先採用 **方案 A（10 台）**，在省下 6 台主機的同時維持 DB 與 RabbitMQ 的獨立性，
...
| 主機角色 | 量 | CPU | RAM | 系統碟 | 資料碟 | Log 碟 | 網路 | 小計 |
|---------|:--:|:---:|:---:|:-----:|:-----:|:------:|:----:|:----:|
| App Server (AA) | 2 | 16C&nbsp;≥3.0&nbsp;GHz | 32&nbsp;GB | 100 GB NVMe | 500 GB NVMe | — | 1 Gbps | 2 |
| DB Server Primary | 1 | 16C | 64&nbsp;GB | — | 1 TB NVMe | 200 GB NVMe | 1 Gbps | 1 |
| DB Server Secondary | 2 | 16C | 64&nbsp;GB | — | 1 TB NVMe | 200 GB NVMe | 1 Gbps | 2 |
...
| **F5 硬體 LB** | **1** | — | — | — | — | — | 10 Gbps | 1 |
| **小計** | **16 台** | | | | | | | |

> TempDB 碟：DB Server 每台另需 50 GB NVMe（可與資料碟同片實體碟分割，或獨立一顆）。
> F5 規格由設備型號決定，請依實際流量需求向 F5 原廠或代理商詢價，建議提供峰值頻寬（10K/sec ≈ 1 Gbps）作為選型依據。
...
| DB Server Primary | 1 | 32C | 128&nbsp;GB | — | 2 TB NVMe | 500 GB NVMe | 10 Gbps | 1 |
| DB Server Secondary | 2 | 32C | 128&nbsp;GB | — | 2 TB NVMe | 500 GB NVMe | 10 Gbps | 2 |
...
> F5 規格由設備型號決定，請依實際流量需求向 F5 原廠或代理商詢價，建議提供峰值頻寬（50K/sec ≈ 10 Gbps）作為選型依據。
